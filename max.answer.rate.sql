...
--max answer rate
with grouping as (select action, question_id, count(*) as cnt from survey_log where action <> 'show' group by action, question_id order by question_id)
,grouping_all_answ as (
    select question_id from grouping group by question_id having count(*) =1 and max(action) = 'answer'
    )
,grouping_none_answ as (
    select question_id from grouping group by question_id having count(*) =1 and max(action) = 'skip'
    )
...
rates as (select question_id, 1.0 as rate from grouping_all_answ union select question_id,0.0 from grouping_none_answ
union select question_id, cnt_a::real/(cnt_a::real+cnt_s::real) from grouping_part_answ order by rate desc limit 1)
select question_id from rates;
